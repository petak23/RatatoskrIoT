...
void nactiHodnotu()
{
  ...
  logger->log("Teplota: %f°C", t);
  logger->log("Vlhkosť: %f%%", h);
...
void setup() {
  ...
  // nadefinujeme kanal
  ch1 = ra->defineChannel( DEVCLASS_CONTINUOUS_MINMAXAVG, 1, "analog_in", 3600 );

  ch2 = ra->defineChannel( DEVCLASS_CONTINUOUS_MINMAXAVG, 2, "analog_in", 3600 );

  // nastavime spousteni mereni jednou za minutu
  tasker.setInterval( nactiHodnotu, 30000 );

  // nacteme jednou hodnotu, aby se prvni nacetla hned a ne az za 30 sekund
  nactiHodnotu();

...
final class DevicePresenter extends BaseAdminPresenter
{
	...
	public function actionEdit(int $id): void
	{
		...
		Logger::log(
			'audit',
			Logger::INFO,
			"Uzivatel #{$this->getUser()->id} {$this->getUser()->getIdentity()->email} edituje/prohlizi konfiguraci/heslo zarizeni {$id}"
		);
	...
	public function renderConfig(int $id): void
	{
		...
		Logger::log(
			'audit',
			Logger::INFO,
			"Uzivatel #{$this->getUser()->id} {$this->getUser()->getIdentity()->email} prohlizi konfiguraci/heslo zarizeni {$id}"
		);
	...
	public function deleteFormSucceeded(Form $form, array $values): void
	{
		...
		if ($id) {
			...
			Logger::log('audit', Logger::INFO, "[{$this->getHttpRequest()->getRemoteAddress()}, {$this->getUser()->getIdentity()->email}] Mazu zarizeni {$id}");
	...
	public function updateFormSucceeded(Form $form, array $values): void
	{
		...
		if ($id) {
			...
			Logger::log(
				'webapp',
				Logger::INFO,
				"Uzivatel #{$this->getUser()->id} {$this->getUser()->getIdentity()->email} posila aktualizaci {$fileId} na zarizeni {$id}"
			);
	...
	public function renderDeleteupdate(int $device_id, int $update_id): void
	{
		...
		Logger::log(
			'webapp',
			Logger::INFO,
			"Uzivatel #{$this->getUser()->id} {$this->getUser()->getIdentity()->email} maze aktualizaci {$update_id} na zarizeni {$device_id}"
		);

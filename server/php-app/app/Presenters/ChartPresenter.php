...
final class ChartPresenter extends BasePresenter
{
	...
	private function checkSensorAccess($sensorUserId, $sensorId)
	{
		if ($this->getUser()->id != $sensorUserId) {
			Logger::log(
				'audit',
				Logger::ERROR,
				"[{$this->getHttpRequest()->getRemoteAddress()}] ACCESS: Uzivatel #{$this->getUser()->id} {$this->getUser()->getIdentity()->email} zkusil pouzit senzor #{$sensorId} patrici jinemu uzivateli"
			);
	...
	public function renderSensorstat(
		...
		} else if ($sensor->id_device_classes == 1) {
